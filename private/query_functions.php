<?php
  ...
  function find_states_for_country_id($country_id=0) {
    global $db;
    $sql = "SELECT * FROM states ";
    $sql .= "WHERE country_id='" . u($country_id) . "' ";
    $sql .= "ORDER BY name ASC;";
    $state_result = db_query($db, $sql);
    return $state_result;
  }
  ...
  function validate_state($state, $errors=array()) {
    ...
    } elseif (!is_valid_name($state['name'])) {
      $errors[] = "Name only contain letters, space and symbol: -";
    }

    if (is_blank($state['code'])) {
      $errors[] = "Code cannot be blank.";
    } elseif (!is_valid_state_code($state['code'])) {
      $errors[] = "Code only allows captital letters";
    } elseif (!is_length_2($state['code'])) {
      $errors[] = "Code number should be two digits long";
    }

    if (is_blank($state['country_id'])) {
      $errors[] = "Country ID cannot be blank.";
    } elseif (!is_only_digits($state['country_id'])) {
      $errors[] = "Country Code only allows numbers";
    } elseif (!is_11_or_less_digits_valid($state['country_id'])) {
      $errors[] = "Country ID must be 11 digits long or less.";
    } 
  ...
  function insert_state($state) {
    ...
    $sql .= "VALUES (";
    $sql .= "'" . sql_stz($db, h($state['name'])) . "',";
    $sql .= "'" . sql_stz($db, h($state['code'])) . "',";
    $sql .= "'" . sql_stz($db, h($state['country_id'])) . "'";
    $sql .= ");";
  ...
  function update_state($state) {
    ...
    $sql .= "name='" . sql_stz($db, h($state['name'])) . "', ";
    $sql .= "code='" . sql_stz($db, h($state['code'])) . "', ";
    $sql .= "country_id='" . sql_stz($db, h($state['country_id'])) . "' ";
    $sql .= "WHERE id='" . sql_stz($db, h($state['id'])) . "' ";
    $sql .= "LIMIT 1;";
  ...
  function validate_territory($territory, $errors=array()) {
    if (is_blank($territory['name'])) {
      ...
      $errors[] = "Name must be between 2 and 255 characters.";
    } elseif (!is_valid_name($territory['name'])) {
      $errors[] = "Name only contain letters, space and symbol: -";
    }

    if (is_blank($territory['state_id'])) {
      $errors[] = "State ID cannot be blank.";
    } elseif (!is_only_digits($territory['state_id'])) {
      $errors[] = "State ID only allows numbers";
    } elseif (!is_11_or_less_digits_valid($territory['state_id'])) {
      $errors[] = "State ID must be 11 digits long or less.";
    } 

    if (is_blank($territory['position'])) {
      $errors[] = "Position cannot be blank.";
    } elseif (!is_only_digits($territory['position'])) {
      $errors[] = "Position only allows numbers";
    } elseif (!is_11_or_less_digits_valid($territory['position'])) {
      $errors[] = "Position must be 11 digits long or less.";
    } 
  ...
  function insert_territory($territory) {
    ...
    $sql .= "VALUES (";
    $sql .= "'" . sql_stz($db, h($territory['name'])) . "',";
    $sql .= "'" . sql_stz($db, h($territory['state_id'])) . "',";
    $sql .= "'" . sql_stz($db, h($territory['position'])) . "'";
    $sql .= ");";
    // For INSERT statments, $result is just true/false
    $result = db_query($db, $sql);
    if($result) {
  ...
  function update_territory($territory) {
    ...
    $sql .= "name='" . sql_stz($db, h($territory['name'])) . "', ";
    $sql .= "state_id='" . sql_stz($db, h($territory['state_id'])) . "', ";
    $sql .= "position='" . sql_stz($db, h($territory['position'])) . "' ";
    $sql .= "WHERE id='" . sql_stz($db, h($territory['id'])) . "' ";
    $sql .= "LIMIT 1;";
  ...
  function find_salespeople_for_territory_id($territory_id=0) {
    ...
    $sql .= "WHERE salespeople_territories.territory_id='" . u($territory_id) . "' ";
    $sql .= "ORDER BY last_name ASC, first_name ASC;";
    $salespeople_result = db_query($db, $sql);
    return $salespeople_result;
  }
  ...
  function validate_salesperson($salesperson, $errors=array()) {
    if (is_blank($salesperson['first_name'])) {
      ...
      $errors[] = "First name only contain letters, space and symbol: -";
    }
    ...
    if (is_blank($salesperson['last_name'])) {
      ...
      $errors[] = "Last name only contain letters, space and symbol: -";
    }

    if (is_blank($salesperson['phone'])) {
      $errors[] = "Phone cannot be blank.";
    } elseif (!is_valid_phone($salesperson['phone'])) {
      $errors[] = "phone only allows letters, numbers, symbols: ( ) -";
    } elseif (!is_phone_length_valid($salesperson['phone'])) {
      $errors[] = "phone number should be either 10 or 11 digits long";
    }
    ...
    if (is_blank($salesperson['email'])) {
      ...
      $errors[] = "email only allows letters, numbers, symbols: @ . _ -";
    }
  ...
  function insert_salesperson($salesperson) {
    ...
    $sql .= "VALUES (";
    $sql .= "'" . sql_stz($db, h($salesperson['first_name'])) . "',";
    $sql .= "'" . sql_stz($db, h($salesperson['last_name'])) . "',";
    $sql .= "'" . sql_stz($db, h($salesperson['phone'])) . "',";
    $sql .= "'" . sql_stz($db, h($salesperson['email'])) . "'";
    $sql .= ");";
    // For INSERT statments, $result is just true/false
    $result = db_query($db, $sql);
    if($result) {
  ...
  function update_salesperson($salesperson) {
    ...
    $sql .= "first_name='" . sql_stz($db, h($salesperson['first_name'])) . "', ";
    $sql .= "last_name='" . sql_stz($db, h($salesperson['last_name'])) . "', ";
    $sql .= "phone='" . sql_stz($db, h($salesperson['phone'])) . "', ";
    $sql .= "email='" . sql_stz($db, h($salesperson['email'])) . "' ";
    $sql .= "WHERE id='" . sql_stz($db, h($salesperson['id'])) . "' ";
    $sql .= "LIMIT 1;";
  ...
  function find_territories_by_salesperson_id($id=0) {
    ...
    $sql .= "WHERE salespeople_territories.salesperson_id='" . u($id) . "' ";
  ...
  function validate_user($user, $errors=array()) {
    if (is_blank($user['first_name'])) {
      ...
      $errors[] = "First name must be between 2 and 255 characters.";
    } elseif (!is_valid_name($user['first_name'])) {
      $errors[] = "First name only contain letters, space and symbol: -";
    }
    ...
    if (is_blank($user['last_name'])) {
      ...
      $errors[] = "Last name must be between 2 and 255 characters.";
    } elseif (!is_valid_name($user['last_name'])) {
      $errors[] = "Last name only contain letters, space and symbol: -";
    ...
    if (is_blank($user['username'])) {
      ...
      $errors[] = "Email only allows letters, numbers, symbol: _";
    }
  ...
  function insert_user($user) {
    ...
    $sql .= "VALUES (";
    $sql .= "'" . sql_stz($db, h($user['first_name'])) . "',";
    $sql .= "'" . sql_stz($db, h($user['last_name'])) . "',";
    $sql .= "'" . sql_stz($db, h($user['email'])) . "',";
    $sql .= "'" . sql_stz($db, h($user['username'])) . "',";
    $sql .= "'" . $created_at . "'";
    $sql .= ");";
    // For INSERT statments, $result is just true/false
    $result = db_query($db, $sql);
  ...
  function update_user($user) {
    ...
    $sql .= "first_name='" . sql_stz($db, h($user['first_name'])) . "', ";
    $sql .= "last_name='" . sql_stz($db, h($user['last_name'])) . "', ";
    $sql .= "email='" . sql_stz($db, h($user['email'])) . "', ";
    $sql .= "username='" . sql_stz($db, h($user['username'])) . "' ";
    $sql .= "WHERE id='" . sql_stz($db, h($user['id'])) . "' ";
    $sql .= "LIMIT 1;";
